...
- [x] **T001** [P] Extend `tests/unit/test_artifacts.py`: N `diff_ready` events
...
- [x] **T002** Implement accumulation in `src/intui/kit/state/artifacts.py`:
...
- [x] **T003** [P] Extend `tests/unit/test_kit_selectors.py`: items with
...
- [x] **T004** Add `parent_id` to `WorkItemView` (`src/intui/kit/state/model.py`,
      defaulted) and set it in `_reduce_item` (`src/intui/kit/state/reduce.py`).
- [x] **T005** Update `tree_view` (`src/intui/kit/state/selectors.py`) to group
...
- [x] **T006** [P] Extend `tests/unit/test_adapter_intentforge.py`:
      ...
      (backward compatible).
- [x] **T007** Update `src/intui/adapters/intentforge.py` (`_item_*`,
      `_file_diff`) to set `scope.task_id` from `suite_id` when present. Make
      T006 pass.

## Phase 4 — Real IF 0.9.13 end-to-end

- [x] **T008** Regenerate `examples/intentforge_console/run.ndjson` from a real
...
- [x] **T009** Extend `tests/integration/test_intentforge_console.py` (Pilot):
...
- [x] **T010** Full gate: `uv run pytest && uv run ruff check && uv run mypy`;
      bump `src/intui/__init__.py` `__version__` to `0.11.0`.
- [x] **T011** [P] Docs: note diff accumulation (+ `reset`) and parent nesting in
      `docs/event-stream-contract.md`; refresh the IF example README if needed.
- [x] **T012** Fresh-build verify: `uv build` + clean-install; spot-check the IF
      example reduces all files + nesting.
- [x] **T013** Merge: `git merge --no-ff` `011-rich-incremental-diffs` into
