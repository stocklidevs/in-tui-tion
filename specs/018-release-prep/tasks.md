...
- [ ] **T001** Rewrite `README.md` as one narrative: tagline + value; install;
...
- [ ] **T002** [P] Add `docs/README.md` (index) linking the quickstart, the
      event-stream contract, and the per-feature quickstarts (009–017).
- [ ] **T003** [P] Add `CHANGELOG.md` (Keep-a-Changelog style) summarizing the
      path to **1.0.0** (highlights per feature area).
- [ ] **T004** [P] Generate a real console screenshot SVG into
      `docs/media/console.svg` and reference it near the top of the README.

## Phase 2 — Packaging metadata

- [ ] **T005** Polish `pyproject.toml`: `keywords`, a full `classifiers` set
...
- [ ] **T006** Verify the wheel includes `py.typed` + the README renders as the
      long description (content-type markdown). `uv build` then `twine check dist/*`.

## Phase 3 — CI

- [ ] **T007** [P] Add `.github/workflows/ci.yml`: on push/PR, set up uv, run
      `uv run pytest`, `uv run ruff check`, `uv run ruff format --check`,
      `uv run mypy` on Python 3.11–3.13 (extras incl. `[metrics]`/dev).

## Phase 4 — Version + final gate

- [ ] **T008** Bump `src/intui/__init__.py` `__version__` to `1.0.0`; `uv sync
      --reinstall-package in-tui-tion`.
- [ ] **T009** Full gate green: `uv run pytest && uv run ruff check && uv run
