...
**Organization**: Tasks are grouped by user story from spec.md. The runnable example grows incrementally â€” each story phase extends it, keeping every phase independently demonstrable (Principle VIII).
...
- [X] T004 [P] Add layering guard test in `tests/unit/test_layering.py` asserting no `textual` import (direct or transitive) in `intui.events/state/viewmodels/actions/theming` â€” this test stays green for the life of the project
...
**Checkpoint**: Event model complete and headlessly tested â€” user story phases can begin.

## Phase 3: User Story 1 â€” Build a TUI from structured state (Priority: P1) ðŸŽ¯ MVP

**Goal**: events â†’ reducers â†’ immutable snapshots â†’ memoized view models â†’ auto-rendering widgets; no imperative repaints.
...
- [ ] T010 [P] [US1] Failing tests for snapshot/reducer in `tests/unit/test_state.py`: structural snapshot equality, monotonic `state_version`, `compose_reducers` slice ownership, unknown event type passes through unchanged, deterministic reduction (same sequence â‡’ equal snapshots)
- [ ] T011 [P] [US1] Failing tests for store in `tests/unit/test_store.py`: ingestâ†’reduceâ†’publish, dedupe via stream, subscriber notification, reducer exception isolated (`ReducerError` in health, prior snapshot retained)
- [ ] T012 [P] [US1] Failing tests for selectors in `tests/unit/test_selectors.py`: memoization per state_version, value-equality of view models, built-in `health_view`

### Implementation for User Story 1

- [ ] T013 [P] [US1] Implement `Snapshot` and `ReducerError` in `src/intui/state/snapshot.py` and `Reducer` protocol + `compose_reducers` in `src/intui/state/reducer.py`
- [ ] T014 [US1] Implement `Store` (ingest, subscribe, snapshot property) in `src/intui/state/store.py` (depends on T013)
- [ ] T015 [P] [US1] Implement `selector` memoization in `src/intui/viewmodels/selector.py` and `health_view` in `src/intui/viewmodels/health.py`
- [ ] T016 [US1] Implement storeâ‡„Textual bridge with render coalescing in `src/intui/widgets/bridge.py` and `BoundWidget` (re-render only on view-model change) in `src/intui/widgets/bound.py`
- [ ] T017 [US1] Implement minimal `IntuiApp` shell (store + source wiring, ingestion as async task, minimal `Theme`/`DEFAULT_THEME` token stubs in `src/intui/theming/theme.py` + `default.py`) in `src/intui/app.py`
- [ ] T018 [US1] Pilot-driven app tests in `tests/snapshot/test_us1_pipeline.py`: initial state renders, new event updates only the bound widget, unknown event type doesn't blank/crash (spec US1 acceptance scenarios)
- [ ] T019 [US1] Create example skeleton `examples/hello_replay/app.py` + `examples/hello_replay/__main__.py` rendering a scrolling item list and status line from a `MemorySource` scripted sequence; runnable via `uv run python -m examples.hello_replay`

**Checkpoint**: MVP â€” a state-driven TUI renders and updates from events, fully testable.

## Phase 4: User Story 2 â€” Replay a recorded session, test headlessly (Priority: P2)
...
- [ ] T020 [P] [US2] Failing tests for recording round-trip in `tests/unit/test_recording.py`: writeâ†’read lossless, malformed line reported with line number + reason, `skip` vs `halt` modes, older-supported-version envelope accepted
- [ ] T021 [P] [US2] Failing determinism suite in `tests/replay/test_determinism.py` with committed fixture `tests/replay/fixtures/sample_run.jsonl`: replay twice â‡’ identical final snapshots; headless replay equals live-run snapshot
...
## Phase 5: User Story 3 â€” React to user input through intents (Priority: P2)
...
- [ ] T027 [P] [US3] Failing tests for confirmation state machine in `tests/unit/test_confirm.py`: createdâ†’confirmingâ†’confirmed delivery, cancelled never delivered, non-risky bypasses confirmation
...
- [ ] T030 [US3] Wire `IntuiApp.post_intent`, keybinding registration for actions, and the built-in confirmation prompt (modal) in `src/intui/app.py`; Pilot tests in `tests/snapshot/test_us3_intents.py` (shortcut â†’ intent delivered with payload; risky â†’ confirm prompt â†’ confirm/cancel paths; all actions keyboard-reachable)
...
**Checkpoint**: the unidirectional loop is closed â€” input â†’ intent â†’ app â†’ events â†’ UI.

## Phase 6: User Story 4 â€” Theme and signal with meaning (Priority: P3)
...
- [ ] T033 [P] [US4] Failing tests for signal status mapping in `tests/unit/test_signal_logic.py` (pure logic: status â†’ StatusStyle resolution, mandatory glyph+label counterpart enforced, unknown status fallback)
...
- [ ] T035 [US4] Implement `Signal` widget with `MotionMode` (steady/pulse/swoosh/strobe) and `StatusStyle` in `src/intui/widgets/signal.py` â€” appearance derived exclusively from bound selector
- [ ] T036 [US4] Pilot + snapshot tests in `tests/snapshot/test_us4_theming.py`: theme switch restyles without widget changes; signal transitions across workingâ†’waitingâ†’succeededâ†’failed; status distinguishable with color disabled (glyph/label present)
...
- [ ] T039 [P] Performance/responsiveness test in `tests/replay/test_responsiveness.py`: 1,000+ event recording at 100 events/sec â€” UI keeps rendering and accepting input (SC-003), renders coalesce (no unbounded queue)
...
- **US3 (Phase 5)**: depends on the app shell (T017). Independent of US2/US4 (pure-logic tasks T026â€“T029 only need Setup).
...
```text
Setup â”€â–¶ Foundational â”€â–¶ US1 (MVP) â”€â”¬â”€â–¶ US2 â”€â”
                                    â”œâ”€â–¶ US3 â”€â”¼â”€â–¶ Polish
                                    â””â”€â–¶ US4 â”€â”˜
```

## Parallel Execution Examples

- **Setup**: T003 âˆ¥ T004 after T002.
- **Foundational**: T005 âˆ¥ T007 (test authoring); T009 after T006/T008.
- **US1**: T010 âˆ¥ T011 âˆ¥ T012 (tests), then T013 âˆ¥ T015 (different modules).
- **After US1**: US2 (T020â€“T025), US3 (T026â€“T031), and US4 (T032â€“T037) can proceed in parallel â€” they touch disjoint modules; only T030/T034 both edit `app.py` (serialize those two).
- **Polish**: T038 âˆ¥ T039 âˆ¥ T040.

## Implementation Strategy

**MVP first**: Phases 1â€“3 deliver a working, tested, state-driven TUI (US1). Stop and demo there if needed.

**Incremental delivery**: each subsequent story lands as an independently testable increment that also extends the single example app â€” after every phase the example still runs and shows strictly more (Principle VIII). Suggested order: US2 next (it makes everything after it cheaper to test), then US3, then US4 â€” matching spec priorities.
